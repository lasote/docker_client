...
#define JSON_F function<void(json11::Json)>
...
class DockerClient{
	public:
		...
		// Containers
		shared_ptr<LambdaRequest> list_containers(JSON_F ret_cb, ERR_F err_cb, bool all=false, int limit=-1, string since="", string before="", int size=-1, Json filters=Json());
		...
		shared_ptr<LambdaRequest> logs_container(STRING_F ret_cb, ERR_F err_cb, string container_id, bool follow=false, bool stdout=true, bool stderr=false, bool timestamps=false, string tail="all");
		shared_ptr<LambdaRequest> create_container(JSON_F ret_cb, ERR_F err_cb, Json parameters);
		...
		shared_ptr<LambdaRequest> attach_to_container(JSON_F ret_cb, ERR_F err_cb, CHAR_PTR_F on_body_cb, string container_id, bool logs=false, bool stream=false, bool stdin=false, bool stdout=false, bool stderr=false);
